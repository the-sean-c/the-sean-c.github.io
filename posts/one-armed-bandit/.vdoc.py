...
def exploit():
    ...
    for i in range(n_iterations):
        if i < n_machines:
            ...
            selection = np.argmax(running_value_estimate[:, i-1, :], axis=1)[:, None]

        print("selection: ", selection.shape)
        print("rewards: ", rewards[:, i, :].shape)
        print("exploit_rewards: ", exploit_rewards[:, i].shape)
        exploit_rewards[:, i] = np.take_along_axis(rewards[:, i, :], selection, axis=1)[:, 0]

        if i > 0:
            running_count[:, i] = running_count[:, i - 1]
        running_count[:, i, selection] += 1
        # print(running_count)
