...
ax = sns.scatterplot(data=rewards_df, x="turn", y="prize", hue="machine", size=3)
plt.grid(axis='y', color='#E5E5E5')
sns.despine(left=True, bottom=True, top=True, right=True)
plt.tick_params(axis='x', which='both', bottom=True, left=True)
plt.legend(loc='upper left', bbox_to_anchor=(1, 1), frameon=False)
...
oracle_e_rewards = np.max(e_rewards, axis=1)
...
```{python}
# | label: fig-average_reward_oracle
# | echo: false
# | fig-cap: "Running Average of Prizes won by Oracle Strategy"
# | fig-alt: "Running Average of Prizes won by Oracle Strategy."

oracle_choice = np.argmax(e_rewards, axis=1)
oracle_rewards = np.take_along_axis(rewards, oracle_choice[:,None], axis=1)[:, 0]
turns = np.array(list(range(1, len(oracle_rewards)+1)))
average_oracle_rewards = np.cumsum(oracle_rewards) / turns
